    ...
    <header className="sticky top-0 z-20 border-b border-black/10 bg-white/70 backdrop-blur supports-[backdrop-filter]:bg-white/60">
      <div className="mx-auto max-w-6xl px-6">
        <div className="flex h-14 items-center justify-between">
          <a href="/" className="text-sm font-semibold">Nowlez.Ai</a>
          <nav className="hidden gap-6 text-sm md:flex">
            {links.map((l) => (
              <a key={l.href} href={l.href} className="text-black/70 hover:text-black">
                {l.label}
              </a>
            ))}
          </nav>
        ...
        {open && (
          <div className="md:hidden">
            <div className="grid gap-2 py-2 text-sm">
              {links.map((l) => (
                <a key={l.href} href={l.href} className="rounded-md px-2 py-2 hover:bg-gray-50" onClick={() => setOpen(false)}>
                  {l.label}
                </a>
              ))}
            </div>
